...
window.openStandardsMatcher = function() {
  ...
  if (!rows.length) {
    ...
    // Open anyway to show existing matches
...
function bsmRunAll(rows) {
  ...
  function runBatch(batchIdx) {
    ...
    // Route via Supabase Edge Function proxy (avoids CORS block on github.io)
    var proxyUrl = (window.SB_NEW_URL || 'https://hbmnlfzvorektbricvsl.supabase.co') +
                   '/functions/v1/claude-proxy';
    fetch(proxyUrl, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ' + (window.SB_NEW_KEY || '')
      },
      body: JSON.stringify({
...
window.bsmSaveApproved = function() {
  ...
  // stdKey already written to boqData in-memory via upd() on approve
  // Just trigger the existing saveToSupabase() which includes stdKey as notes
  var statusEl = document.getElementById('bsm-save-status');
  statusEl.textContent = approved.length + ' תקנים מאושרים — שומר ל-Supabase...';
  bsmResults.forEach(function(r) {
    if (r.status === 'approved') r.status = 'saved';
  });
  bsmRender();
  if (typeof saveToSupabase === 'function') {
    saveToSupabase();
  } else {
    if (typeof toast === 'function') toast('שגיאה: saveToSupabase לא נמצא','err');
  }
};
